...
loginRoutes.post('/', async (req, res) => {
  ...
  const token = jwt.sign({ id: user.id, email: user.email, name: user.name }, process.env.REFRESH_TOKEN_SECRET, {
    expiresIn: '1d',
  });

  res.cookie('access_token', token, { expires: new Date(Date.now() + 1000 * 60 * 60 * 24) });

  res.status(200).json({ id: user.id, email: user.email, name: user.name, accessToken: token });
});
